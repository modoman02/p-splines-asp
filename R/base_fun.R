base_fun <- function(x, knots, m, j) { # x = data, knots = list of knots, l = degree, m = num of knots in interval [a,b], j = current knot
  ...
  if (tp) {
    ...
    term1 <- if (denom1 == 0) {0} else ((x - knots[j]) / denom1) * base_fun(x, list(knots = knots, tp = tp), l - 1, m, j)
    term2 <- if (denom2 == 0) {0} else ((knots[j + l + 1] - x) / denom2) * base_fun(x, list(knots = knots, tp = tp), l - 1, m, j + 1)
    return(term1 + term2)
  }
}
